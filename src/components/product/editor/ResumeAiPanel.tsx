...
import { ChevronDown, LoaderCircle, Sparkles, type LucideIcon } from "lucide-react";
...
function ResumeAiDisclosure({
  ...
  return (
    <details className="resume-ai-disclosure" {...(defaultOpen ? { open: true } : {})}>
      <summary className="resume-ai-disclosure-summary">
        <span className="resume-ai-disclosure-heading">
          {Icon ? <Icon className="size-4" /> : null}
          <span>{title}</span>
        </span>
        <span className="resume-ai-disclosure-meta">
          {badge}
          <ChevronDown className="resume-ai-disclosure-chevron size-4" />
        </span>
      </summary>

      <div className="resume-ai-disclosure-body">{children}</div>
    </details>
  );
}
  ...
  return (
    <section className="resume-editor-panel">
      ...
      <div className="resume-editor-group">
        <ResumeAiDisclosure
          badge={<Badge tone={usesRemoteProvider ? "accent" : "neutral"}>{usesRemoteProvider ? "远程模型" : "本地模式"}</Badge>}
          icon={Sparkles}
          title="模型设置"
        >
          <AiConfigForm
        ...
        <ResumeAiDisclosure
          badge={<Badge tone={generatedSummarySuggestions.length > 0 ? "success" : "neutral"}>{generatedSummarySuggestions.length} 条建议</Badge>}
          icon={Sparkles}
          title="摘要润色"
        >
      ...
      <div className="resume-editor-group">
        <ResumeAiDisclosure
          badge={<Badge tone={analysis.missingKeywords.length > 0 ? "warning" : "success"}>{coverageLabel}</Badge>}
          icon={Sparkles}
          title="岗位分析"
        >
          ...
          {!analysis.active ? (
            <div className="empty-surface empty-surface-left resume-ai-empty-state">
              ...
              <p className="empty-surface-text">填入后这里才会出现分析结果。</p>
            </div>
          ) : (
      ...
      <div className="resume-editor-group">
        <ResumeAiDisclosure
          badge={<Badge tone={tailoredPlan.canGenerate ? "accent" : "warning"}>{tailoredPlan.keptSections} 个模块</Badge>}
          icon={Sparkles}
          title="定制版生成"
        >
